...
const TIERS = [
  { key: "EARLYBIRD", nickname: "Early bird", unit_amount: 900 },
  { key: "NORMAL", nickname: "Normal", unit_amount: 1400 },
];
...
async function main() {
  ...
  for (const p of PRODUCTS) {
    ...
    for (const t of TIERS) {
      const price =
        existing.find((x) => x.unit_amount === t.unit_amount && x.currency === "eur") ??
