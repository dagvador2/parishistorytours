    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div>
          {selectedDay && (
            <div>
              <h4 className="font-semibold text-gray-800 mb-4">
                ...
                {selectedDay.toLocaleDateString(locale)} {t.calendar.for}{" "}
                {booking.participants} {participantsLabel}.
              </h4>
              {slots.length > 0 ? (
                      ...
                      >
                        <div className="flex justify-between items-center">
                          <div>
                            ...
                            <div
                              className={`text-sm ${
                                manySpots ? "text-green-600 font-medium" : "text-gray-600"
                              }`}
                            >
                          ...
                          <div className="text-right">
                            ...
                            {isSelected && !priceLoading && (
                              <div className="text-sm text-blue-600">
                                {t.calendar.total} €
                                {pricePerPerson * booking.participants}
                              </div>
                            )}
                          </div>
                        </div>
          ...
          {!selectedDay && (
            <p className="text-gray-500 text-sm">{t.calendar.selectDate}</p>
          )}
        </div>
      </div>
      ...
      {booking.sessionId && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          ...
          <p className="text-lg font-bold text-green-800 mt-2">
            {t.calendar.total} €{booking.price} {t.calendar.for}{" "}
            {booking.participants} {participantsLabel}
          </p>
        </div>
      )}
